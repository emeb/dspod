 ...
 * 05-06-29 E. Brombaugh - updated for dspod_u3c5
 */
...
#define ST7789_BL_LOW()    (gp_out&=~(1<<29))
#define ST7789_BL_HIGH()   (gp_out|=(1<<29))
...
/*
 * fast color fill - Hardened SPI core is only 8bit max
 */
...
void ST7789_setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
	...
	x0 += colstart;
	ST7789_write_byte(x0>>8);
	ST7789_write_byte(x0&0xff);     // XSTART
	x1 += colstart;
	ST7789_write_byte(x1>>8);
	ST7789_write_byte(x1&0xff);     // XEND

	ST7789_write_byte(ST77XX_RASET | ST_CMD); // Row addr set
	y0 += rowstart;
	ST7789_write_byte(y0>>8);
	ST7789_write_byte(y0&0xff);     // YSTART
	y1 += rowstart;
	ST7789_write_byte(y1>>8);
	ST7789_write_byte(y1&0xff);     // YEND
...
void ST7789_bitblt(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t *buf)
{
	...
	ST7789_DC_DATA();
	spi_cs_low(st7789_spi, ST7789_CSBIT);
	spi_transmit(st7789_spi, (uint8_t *)buf, h*w*sizeof(uint16_t));
	spi_cs_high(st7789_spi);
}
 ...
 * low power states
 */
void ST7789_sleep(uint8_t ena)
{
	uint16_t cmd = ena ? ST77XX_SLPIN : ST77XX_SLPOUT;
	ST7789_write_byte(cmd | ST_CMD);
}
