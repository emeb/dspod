/*
 * si5351.c - SI5351 Clock Generator driver for dspod_up5k
 ...
 * I2C stuff
 */
#define SI5351_I2C_ADDRESS   0x60	// 110000+SA0 - 0x60 or 0x61, shifted left
 ...
 */
uint8_t si5351_i2c_write(uint8_t reg, uint8_t *data, uint8_t sz)
{
	uint8_t i, i2c_msg[9];
	...
	return i2c_mtx(I2C0, SI5351_I2C_ADDRESS, i2c_msg, sz+1);
}

/*
 * Get a block of data bytes from the Si5351 via I2C
 */
uint8_t si5351_i2c_read(uint8_t reg, uint8_t *data, uint8_t sz)
{
	uint8_t status = 0;
	
	/* send register */
	status = i2c_mtx(I2C0, SI5351_I2C_ADDRESS, &reg, 1);
	
	/* get data */
	if(status == 0)
		status = i2c_mrx(I2C0, SI5351_I2C_ADDRESS, data, sz);
