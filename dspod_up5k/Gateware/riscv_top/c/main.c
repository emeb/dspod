/*
 * main.c - top level of picorv32 firmware for dspod_up5k
 * 06-04-26 E. Brombaugh
 */
#include "main.h"
#include "acia.h"
#include "spi.h"
#include "flash.h"
#include "psram.h"
#include "clkcnt.h"
#include "st7789.h"
#include "i2c.h"
#include "codec.h"
#include "si5351.h"

/* build time */
const char *bdate = __DATE__;
const char *btime = __TIME__;
const char *fwVersionStr = "V0.1";

uint8_t dest[32768] __attribute__ ((section (".ram1_data")));
void (*loaded_code)(void) = dest;

/*
 * simple text output to graphics
 */
uint8_t gline = 0;
static void putcp(void* p,char c)
{
    *(*((char**)p))++ = c;
}

void gprintf(char *fmt, ...)
{
	char s[32], *h = s;
    va_list va;
    va_start(va,fmt);
    tfp_format(&h,putcp,fmt,va);
    putcp(&h,0);
    va_end(va);
	st7789_drawstr(0, 8*gline++, s, ST7789_WHITE, ST7789_BLACK);
}

/*
 * main
 */
void main()
{
	uint32_t cnt, spi_id, i, j;
	//int c;
	
	init_printf(0, acia_printf_putc);
	printf("\n\n\r-----------------------------\n\r");
	printf("dspod_up5k riscv_top - starting up\n\r");
    printf("Version: %s\n\r", fwVersionStr);
    printf("Build Date: %s\n\r", bdate);
    printf("Build Time: %s\n\r", btime);
	printf("-----------------------------\n\r");

	/* test both SPI ports */
	spi_init(SPI0);
	spi_init(SPI1);
	
#if 0
	/* Init LCD */
	st7789_init(SPI1);
	printf("LCD initialized\n\r");
#endif
	
#if 0
	/* test lcd text */
	for(i=0;i<21;i++)
	{
		gprintf("Line %d", i);
	}
#endif

#if 0
	/* get spi flash id */
	flash_init(SPI0);	// wake up the flash chip
	spi_id = flash_id(SPI0);
	printf("spi flash id: 0x%08X\n\r", spi_id);
	
	printf("spi flash SR1: %02X\n\r", flash_rdreg(SPI0, FLASH_RSR1));
	printf("spi flash SR2: %02X\n\r", flash_rdreg(SPI0, FLASH_RSR2));
	printf("spi flash SR3: %02X\n\r", flash_rdreg(SPI0, FLASH_RSR3));
	
	/* read some data */
	{
		uint8_t read[128];
		flash_read(SPI0, read, 0x000000, 128);
		for(i=0;i<16;i+=4)
		{
			printf("%02X: %02X %02X %02X %02X\n\r", i, read[i], read[i+1], read[i+2], read[i+3] );
		}
	}
#endif

#if 1
	/* copy flash to RAM */
	flash_init(SPI0);	// wake up the flash chip
	printf("dest = 0x%08X\n\r", dest);
	flash_read(SPI0, dest, 0x100000, 0x8000);
	for(i=0;i<256;i+=16)
	{
		printf("%02X: ", i);
		for(j=0;j<16;j++)
			printf("%02X ", dest[i+j]);
		printf("\n\r");
	}
	printf("Jumping to loaded code @ 0x%08X...\r\n\n\n", loaded_code);
	loaded_code();
#endif

#if 0
	/* get spi psram id */
	spi_id = psram_id(SPI0);
	printf("spi psram id: 0x%08X\n\r", spi_id);
	gprintf("SPI PSRAM Test...");
	gprintf("ID: 0x%08X", spi_id);
	
	/* read some data */
	{
		uint8_t read[128];
		psram_read(SPI0, read, 0, 128);
		gprintf("read");
		for(i=0;i<16;i+=4)
		{
			gprintf("%02X: %02X %02X %02X %02X", i, read[i], read[i+1], read[i+2], read[i+3] );
		}
		
		gprintf("write");
		for(i=0;i<16;i++)
			read[i] = i<<4 | 0xf;
		for(i=0;i<16;i+=4)
		{
			gprintf("%02X: %02X %02X %02X %02X", i, read[i], read[i+1], read[i+2], read[i+3] );
		}
		psram_write(SPI0, read, 0, 128);
		
		gprintf("zero");
		for(i=0;i<16;i++)
			read[i] = 0;
		for(i=0;i<16;i+=4)
		{
			gprintf("%02X: %02X %02X %02X %02X", i, read[i], read[i+1], read[i+2], read[i+3] );
		}
		
		psram_read(SPI0, read, 0, 128);
		gprintf("re-read");
		for(i=0;i<16;i+=4)
		{
			gprintf("%02X: %02X %02X %02X %02X", i, read[i], read[i+1], read[i+2], read[i+3] );
		}
	}
#endif

#if 0
	/* test limits */
	st7789_fillScreen(ST7789_BLACK);
#define XMIN 0
#define XMAX st7789_width()
#define YMIN 0
#define YMAX st7789_height()
	st7789_drawFastVLine(XMIN, YMIN, YMAX, ST7789_WHITE);
	st7789_drawFastVLine(XMAX-1, YMIN, YMAX, ST7789_WHITE);
	st7789_drawFastHLine(XMIN, YMIN, XMAX-XMIN, ST7789_WHITE);
	st7789_drawFastHLine(XMIN, YMAX-1, XMAX-XMIN, ST7789_WHITE);
	st7789_drawLine(XMIN, YMIN, XMAX, YMAX, ST7789_WHITE);
	st7789_drawLine(XMAX, YMIN, XMIN, YMAX, ST7789_WHITE);
	st7789_drawstr(XMAX/2-44, (YMAX/2-4), "Hello World",
		ST7789_WHITE, ST7789_BLACK);
	clkcnt_delayms(1000);
#endif
	
#if 0
	/* color fill + text fonts */
	printf("Color Fill & Fonts\n\r");
	st7789_fillScreen(ST7789_MAGENTA);
	st7789_drawstr(st7789_width()/2-44, (st7789_height()/2-12*8), "Hello World", ST7789_WHITE, ST7789_MAGENTA);
	
	/* test font */
	for(i=0;i<256;i+=16)
		for(j=0;j<16;j++)
			st7789_drawchar((st7789_width()/2-8*8)+(j*8), (st7789_height()/2-8*8)+(i/2), i+j,
				ST7789_GREEN, ST7789_BLACK);
	
	clkcnt_delayms(1000);
#endif
	
#if 0
	/* test colored lines */
	printf("Colored Lines\n\r");
	{
		uint8_t rgb[3], hsv[3];
		uint16_t color;
		st7789_fillScreen(ST7789_BLACK);
		hsv[1] = 255;
		hsv[2] = 255;
		j=256;
		while(j--)
		{	
			for(i=0;i<st7789_width();i++)
			{
				hsv[0] = (i+j);
				
				st7789_hsv2rgb(rgb, hsv);
				color = st7789_Color565(rgb[0],rgb[1],rgb[2]);
		#if 1
				/* rotating box */
				st7789_drawLine(i, 0, st7789_width()-1, i, color);
				st7789_drawLine(st7789_width()-1, i, st7789_width()-1-i, st7789_width()-1, color);
				st7789_drawLine(st7789_width()-1-i, st7789_width()-1, 0, st7789_width()-1-i, color);
				st7789_drawLine(0, st7789_width()-1-i, i, 0, color);
		#else
				/* slow downward motion */
				st7789_drawFastHLine(0, i, st7789_width(), color);
		#endif
			}
		}
	}
	clkcnt_delayms(1000);
#endif

#if 0
	/* test image blit from flash */
	printf("Image from flash\n\r");
	{
#define LINESPERBLIT 16
		flash_init(SPI0);	// wake up the flash chip
		uint16_t blit[ST7789_TFTHEIGHT*LINESPERBLIT];
		uint32_t blitaddr, blitsz;
		blitaddr = 0xa0000;
		blitsz = ST7789_TFTHEIGHT*LINESPERBLIT*sizeof(uint16_t);
		for(i=0;i<ST7789_TFTWIDTH;i+=LINESPERBLIT)
		{
			flash_read(SPI0, (uint8_t *)blit, blitaddr, blitsz);
			st7789_blit(0, i, ST7789_TFTHEIGHT, LINESPERBLIT, blit);
			blitaddr += blitsz;
		}
	}
#endif

#if 0
	/* Test I2C */
	i2c_init(I2C0);
	printf("I2C0 Initialized\n\r");
#if 0
	uint8_t i2c_add = 0x1A;
	uint8_t i2c_msg[] = {0x00, 0x00};
	printf("i2c_mtx(0x%02X) = %d\n\r", i2c_add, i2c_mtx(I2C0, i2c_add, i2c_msg, 2));
#endif
#endif

#if 0
	/* init codec */
	if(Codec_Init() == 0)
	{
		printf("Codec initialized\n\r");
		
#if 0
		/* dump regs to test */
		for(i=0;i<70;i++)
		{
			uint16_t dat;
			uint8_t stat;
			if((stat = Codec_ReadRegister(i, &dat)) == 0)
				printf("Codec Reg %2d = 0x%03X\n\r", i, dat);
			else
				printf("Failed read @ Reg %2d, stat = %d\n\r", i, stat);
		}
#endif
	}
	else
		printf("Codec init failed!\n\r");
#endif

#if 0
	/* init Si5351 */
	if(si5351_init() == 0)
	{
		si5351_set_output_chl(0, 12288000, 0);
		printf("SI5351 initialized\n\r");
	}
#endif
	
	cnt = 0;
	printf("Looping...\n\r");
	while(1)
	{
		/* LED cycle */
		gp_out = (gp_out&~(7<<17))|((cnt&7)<<17);
		cnt++;
		
#if 0
		printf("gp_in: 0x%03X\n\r", gp_in);
#endif
		
#if 0
		/* ADC input test */
		printf("adc: %d %d %d %d\n\r", ADC->CHL0, ADC->CHL1, ADC->CHL2, ADC->CHL3);
#endif

#if 0
		/* master transmit test */
		if(i2c_mtx(I2C0, 0x1A, (uint8_t *)&cnt, 2))
			acia_putc('x');
		else
			acia_putc('.');
#endif

#if 0
		/* slave receive (blocking) test */
		{
			uint16_t data;
			if(i2c_srx(I2C0, 0x19, (uint8_t *)&data, 2)&1)
			{
				printf("----\n\r");
			}
			else
			{
				printf("0x%04X\n\r", data);
			}
		}
#endif
		
		
#if 0
		/* simple echo */
		if((c=acia_getc()) != EOF)
			acia_putc(c);
#endif		
		clkcnt_delayms(200);
	}
}
