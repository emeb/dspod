...
/* external memory buffer in PSRAM */
int16_t *fx_ext_buffer;
size_t fx_ext_sz;

/* pre-allocated internal memory */
size_t fx_int_sz;
uint32_t *fx_mem;
 ...
 */
void IRAM_ATTR fx_bypass_Proc(void *dummy, int16_t *dst, int16_t *src, uint16_t sz)
{
	/* just copy in to out */
	while(sz--)
	{
...
uint8_t fx_init(void)
{
	...
	if(fx_mem)
		ESP_LOGI(TAG, "%d bytes internal allocated for audio", FX_MAX_MEM);
	else
	...
	ESP_LOGI(TAG, "%d bytes PSRAM available", fx_ext_sz);
	fx_ext_buffer = heap_caps_malloc(fx_ext_sz, MALLOC_CAP_SPIRAM);
	if(fx_ext_buffer)
		ESP_LOGI(TAG, "%d bytes PSRAM allocated for audio buffers", fx_ext_sz);
	else
