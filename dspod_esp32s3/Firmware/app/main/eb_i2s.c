...
#define GPIO_TX_DIAG_PIN        GPIO_NUM_2       // Realtime Diag pin for TX
#define GPIO_RX_DIAG_PIN        GPIO_NUM_3       // Realtime Diag pin for RX
...
bool i2s_async_rx_cb(i2s_chan_handle_t handle, i2s_event_data_t *event, void *user_ctx)
{
	 ...
	 * but prevents miscalcs in foreground process (like sine table compute
	 * in morph init) - Worth it?
	 */
	xthal_set_cpenable(1);
	//xthal_save_cp0(cp0_regs);
	
	/* get buffer & size */
	rx_buffer = (int16_t *)event->dma_buf; // new API for 5.5.2
...
bool i2s_async_tx_cb(i2s_chan_handle_t handle, i2s_event_data_t *event, void *user_ctx)
{
	...
	tx_buffer = (int16_t *)event->dma_buf;	// new API for 5.5.2
