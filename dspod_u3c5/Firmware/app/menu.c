/*
 * menu.c - menuing UI for dspod_rp2350
...
void menu_render_wetdry(void)
{
	...
	int16_t curr_wetdry = ADC_val[3]/41;
	char textbuf[32];
...
{
	/*
	 * for some reason this is needed to wakeup SPI for menu init
	 * delays from initial splash screen > 30ms seem to cause problems
	 * w/o this (or any other SPI operation).
	 */
	Shared_SPI_WriteByte(0); // try just naked SPI op w/o asserting LCD CS
