...
#define PSRAM_BUFSZ 64
	mem += psbuf_init(&blk->wpsb, mem, PSRAM_BUFSZ, 0, OSPI_PSRAM_SIZE);
	mem += psbuf_init(&blk->r1psb, mem, PSRAM_BUFSZ, 0, OSPI_PSRAM_SIZE);
	mem += psbuf_init(&blk->r2psb, mem, PSRAM_BUFSZ, 0, OSPI_PSRAM_SIZE);
#endif
	...
	blk->len = OSPI_PSRAM_SIZE / (2*sizeof(int16_t));	// length in samples
	blk->init = 1;
	blk->wptr = 0;
#ifdef USE_PSRAMBUF
	...
	printf("fx_cdx_Init -------\n\r");
	printf("\tOSPI_SIZE = %d, len = 0x%08X\n\r", OSPI_PSRAM_SIZE, blk->len);
	for(int i=0;i<3;i++)
 ...
 */
void fx_cdx_Proc(void *vblk, int16_t *dst, int16_t *src, uint16_t sz)
{
	fx_cdx_blk *blk = vblk;
	uint16_t i;
	int16_t fb_lvl;
	...
	for(i=0;i<sz;i++)
	{
		...
		psbuf_writebytes(&blk->wpsb, 4*blk->wptr, (uint8_t *)rl_out,
			2*sizeof(int16_t));
#else
		xspi_psram_swritebytes(4*blk->wptr, (uint8_t *)rl_out, 2*sizeof(int16_t));
#endif
		...
		psbuf_readbytes(&blk->r1psb, 4*rptr, (uint8_t *)&rl_out,
			2*sizeof(int16_t));
#else
			xspi_psram_readbytes(4*rptr, (uint8_t *)&rl_out, 2*sizeof(int16_t));
#endif
		else
			rl_out[0] = rl_out[1] = 0;
		...
		if(blk->xfcnt)
		{
			...
			if(!blk->init || (rptr < blk->wptr))
			{
				...
				psbuf_readbytes(&blk->r2psb, 4*rptr, (uint8_t *)&rl_xf,
					2*sizeof(int16_t));
#else
				xspi_psram_readbytes(4*rptr, (uint8_t *)rl_xf, 2*sizeof(int16_t));
#endif
				for(chl=0;chl<2;chl++)
				{
