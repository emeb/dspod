/*
 * psram_buff.h - read & write buffering for the ospi psram indirect mode
 * 11-22-2025 E. Brombaugh
 * 05-09-2026 E. Brombaugh - updated for stm32u3c5s
 */
	...
	uint32_t sz)
{
	...
	if((addr != pbf->next_addr) && (pbf->fill_cnt != 0))
	{
		...
		ospi_psram_writebytes(pbf->psram_start + pbf->begin_addr, pbf->buf,
			pbf->fill_cnt);
		pbf->fill_cnt = 0;
		pbf->next_addr = addr;
	}
	...
	while(sz--)
	{
		...
		if(pbf->fill_cnt == pbf->bufsz)
		{
			...
			{
				/* write w/o wrap */
				ospi_psram_writebytes(psram_addr, pbf->buf, pbf->fill_cnt);
			...
			else
			{
				...
				ospi_psram_writebytes(psram_addr, pbf->buf, tlen);
	...
	uint32_t sz)
{
	...
	if((addr != pbf->next_addr) || (pbf->fill_cnt == 0))
	{
		...
		{
			/* no wrap */
			ospi_psram_readbytes(psram_addr, pbf->buf, pbf->bufsz);
		...
		else
		{
			...
			uint32_t tlen = pbf->psram_sz - psram_addr;
			ospi_psram_readbytes(psram_addr, pbf->buf, tlen);
			pbf->fill_cnt = tlen;
			pbf->begin_addr += tlen;
		}
