/*
 * main.c - dspod_h7r3 i2s_codec test program
 * 11-02-25 E. Brombaugh
 ******************************************************************************
 * Changelog
 *
 * date: 2025-11-01 V0.0
 * Initial creation
 *
 */
...
int main(void)
{
	...
	printf("Build Time: %s\n\r", btime);
	printf("\n");
	clkfrq = HAL_RCC_GetSysClockFreq();
	printf("SYSCLK = %d\n\r", clkfrq);
	printf("\n");
	...
	/* Infinite loop */
	printf("Looping...\n\r");
	clkfrq = clkfrq/1000000;
	dg = cyclegoal_ms(DELAY);
	while(1)
	{
		if(!cyclecheck(dg))
		{
			dg = cyclegoal_ms(DELAY);
			get_meas(&act, &tot);
			printf("CNT 0x%08X, period %d us, CPU Load %d / %d (%d%%)   \r", cyclegoal(0), tot/clkfrq, act, tot, (100*act)/tot);
			LEDToggle();
		}
	}
