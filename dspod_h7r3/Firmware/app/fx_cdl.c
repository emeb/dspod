...
void * fx_cd_common_Init(uint32_t *mem, uint8_t type)
{
...
#ifdef USE_MMPSR
	xspi_memmap(1);
	blk->dlybuf = (int16_t *)XSPI1_BASE;	// mem mapped PSRAM via XSPI1
	...
	printf("\ttype = %d, rng = %d, dlybuf = 0x%08X, len = 0x%08X\n\r",
		blk->type, blk->rng, blk->dlybuf, blk->len);
 ...
 */
void __attribute__ ((section (".itcm_code"))) fx_cd_common_Proc(void *vblk, int16_t *dst, int16_t *src, uint16_t sz)
{
	fx_cdl_blk *blk = vblk;
	uint16_t i;
	int16_t fb_lvl;
